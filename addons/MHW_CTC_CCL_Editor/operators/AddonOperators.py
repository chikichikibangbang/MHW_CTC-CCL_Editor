...
class SwitchToPoseMode(bpy.types.Operator):
    ...
    bl_description = "Switch to Pose Mode to add new ctc chains and ccl collisions"
...
class SwitchToObjectMode(bpy.types.Operator):
    ...
    bl_description = "Switch to Object Mode to configure ctc chains and ccl collisions"
...
class CCLHideNonCollisions(bpy.types.Operator):
    ...
    bl_description = "Hide all objects that aren't ccl collisions to make selecting and configuring them easier." \
                     "\nPress the \"Unhide All\" button to unhide"
...
class CTCHideNonAngleLimits(bpy.types.Operator):
    ...
    bl_description = "Hide all objects that aren't angle limits to make selecting and configuring them easier." \
                     "\nPress the \"Unhide All\" button to unhide"
...
class CTCApplyAngleLimitRamp(bpy.types.Operator):
    ...
    bl_description = "Apply an increasing angle limit radius on each ctc node as it gets further away.\nA ctc chain must be selected. If multiple ctc chains are selected, it will be applied to all of them"
...
class CTCRenameAndTransferBone(bpy.types.Operator):
    ...
    def draw(self, context):
        ...
        if language_code == "zh_CN" or language_code == "zh_TW":
            row.label(text="未使用ID数量 (150~200): " + str(boneIDnum))
        else:
            row.label(text="number of unused ID (150~200): " + str(boneIDnum))
        # row = col.row(align=True)
        # row.label(text=str(boneIDnum))
        row = col.row(align=True)
...
class CTCTransferBoneSettings(bpy.types.Operator):
    ...
    bl_description = "Settings for detailed operation items"
...
class CTCApplyCTCChainPreset(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        for activeObj in bpy.context.selected_objects:
            if activeObj.get("TYPE", None) == "CTC_CHAIN":
        ...
        if finished:
            ...
            showErrorMessageBox("Must select a ctc chain in order to apply the preset to it.")
            return {'CANCELLED'}
