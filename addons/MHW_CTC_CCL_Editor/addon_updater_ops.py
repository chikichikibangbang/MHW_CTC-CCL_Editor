...
updater.addon = "mhw_ctc_ccl_editor"
...
def get_user_preferences(context=None):
    ...
    if hasattr(context, "user_preferences"):
        prefs = context.user_preferences.addons.get(addon_name, None)
    elif hasattr(context, "preferences"):
        prefs = context.preferences.addons.get(addon_name, None)
    if prefs:
...
def register(bl_info):
    ...
    updater.user = "chikichikibangbang"

    # Choose your own repository, must match git name for GitHUb and Bitbucket,
    # for GitLab use project ID (numbers only).
    updater.repo = "MHW_CTC_CCL_Editor"

    # updater.addon = # define at top of module, MUST be done first

    # Website for manual addon download, optional but recommended to set.
    updater.website = "https://github.com/chikichikibangbang/MHW_CTC_CCL_Editor"
    ...
    # 			/addons/{addon_name}/{addon_name}_updater
